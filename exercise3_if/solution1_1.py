...
feb_mday_index = 0 #定义feb_mday_index为假
...
for i in range(today_stime.tm_mon-1): #让i在数列today_stime.tm_mon-1(用户输入的月份减一)
    if i == 1: #如果i等于1 
        days = days + mdays[i][feb_mday_index]  #days等于days加二月份的29天
    else: #否则
        days = days + mdays[i] #days等于days加二月份的28天
days = days + today_stime.tm_mday  #days等于加上用户输入的号数
#s_today = '这一天为' + str(today_stime.tm_year) + '年的第' + str(days) + '天。'
s_today = "这一天为{}年的第{}天。".format(today_stime.tm_year, days) #连接字符串，并储存到s_today中
print(s_today) #打印
